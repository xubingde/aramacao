...
WtTplCtorFn::WtTplCtorFn(QWidget *  parent /* = nullptr */):
        WtBase(parent),
        m_objPtr(nullptr),
        ...
        m_isNoexcept(nullptr)
{
    ...
    connect(m_baseClassDefVal, &QLineEdit::editingFinished,
            this, &WtTplCtorFn::baseClassDefVal_editingFinished);
    connect(m_attribute, &QLineEdit::editingFinished,
            this, &WtTplCtorFn::attribute_editingFinished);
    connect(m_beforBehindPb, &QPushButton::clicked,
            this, &WtTplCtorFn::beforBehindPb_clicked);
    connect(m_codeEdit, &CodeEditor::textChanged,
            this, &WtTplCtorFn::codeEdit_textChanged);
    ...
    connect(m_fieldValView->itemDelegate(),
            ...
            this, &WtTplCtorFn::fieldVal_itemDelegate_closeEditor);
    connect(m_baseClassValView->itemDelegate(),
            &QAbstractItemDelegate::closeEditor,
            this, &WtTplCtorFn::baseClassVal_itemDelegate_closeEditor);

    connect(m_isEnabled, &QCheckBox::stateChanged,
            this, &WtTplCtorFn::isEnabled_stateChanged);
    connect(m_isExplicit, &QCheckBox::stateChanged,
            this, &WtTplCtorFn::isExplicit_stateChanged);
    connect(m_isNoexcept, &QCheckBox::stateChanged,
            this, &WtTplCtorFn::isNoexcept_stateChanged);
...
WtTplCtorFn::~WtTplCtorFn() noexcept
{
}

void
WtTplCtorFn::baseClassDefVal_editingFinished()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::attribute_editingFinished()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::beforBehindPb_clicked()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::codeEdit_textChanged()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::parameterItemConnect()
{
    ...
    connect(actAddNew, &QAction::triggered,
            this, &WtTplCtorFn::param_AddNew_triggered);
    connect(actInsertNew, &QAction::triggered,
            this, &WtTplCtorFn::param_InsertNew_triggered);
    connect(actDelete, &QAction::triggered,
            this, &WtTplCtorFn::param_Delete_triggered);
    connect(actCopyToNew, &QAction::triggered,
            this, &WtTplCtorFn::param_CopyToNew_triggered);
    connect(actUp, &QAction::triggered,
            this, &WtTplCtorFn::param_Up_triggered);
    connect(actDown, &QAction::triggered,
            this, &WtTplCtorFn::param_Down_triggered);
    connect(actMoveToRow, &QAction::triggered,
            this, &WtTplCtorFn::param_MoveToRow_triggered);

    connect(actBeforBehind, &QAction::triggered,
            this, &WtTplCtorFn::param_BeforBehind_triggered);

    connect(m_parameterView->itemDelegate(),
            &QAbstractItemDelegate::closeEditor,
            this, &WtTplCtorFn::param_itemDelegate_closeEditor);
}

void
WtTplCtorFn::param_AddNew_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_InsertNew_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_Delete_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_CopyToNew_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_Up_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_Down_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_MoveToRow_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_BeforBehind_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::param_itemDelegate_closeEditor()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::isEnabled_stateChanged(int  status)
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::isExplicit_stateChanged(int  status)
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::isNoexcept_stateChanged(int  status)
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::fieldVal_itemDelegate_closeEditor()
{
...
void
WtTplCtorFn::baseClassVal_itemDelegate_closeEditor()
{
...
void
WtTplCtorFn::paramTplItemConnect()
{
    QAction *  actAddNew = new QAction(tr("Add New"));
    QAction *  actInsertNew = new QAction(tr("Insert New"));
    QAction *  actDelete = new QAction(tr("Delete"));
    ...
    connect(actAddNew, &QAction::triggered,
            this, &WtTplCtorFn::paramTpl_AddNew_triggered);
    connect(actInsertNew, &QAction::triggered,
            this, &WtTplCtorFn::paramTpl_InsertNew_triggered);
    connect(actDelete, &QAction::triggered,
            this, &WtTplCtorFn::paramTpl_Delete_triggered);
    connect(actCopyToNew, &QAction::triggered,
            this, &WtTplCtorFn::paramTpl_CopyToNew_triggered);
    connect(actUp, &QAction::triggered,
            this, &WtTplCtorFn::paramTpl_Up_triggered);
    connect(actDown, &QAction::triggered,
            this, &WtTplCtorFn::paramTpl_Down_triggered);
    connect(actMoveToRow, &QAction::triggered,
            this, &WtTplCtorFn::paramTpl_MoveToRow_triggered);
    ...
    connect(m_paramTplView->itemDelegate(),
            &QAbstractItemDelegate::closeEditor,
            this, &WtTplCtorFn::paramTpl_itemDelegate_closeEditor);
}

void
WtTplCtorFn::paramTpl_AddNew_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::paramTpl_InsertNew_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::paramTpl_Delete_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::paramTpl_CopyToNew_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::paramTpl_Up_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::paramTpl_Down_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::paramTpl_MoveToRow_triggered()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::paramTpl_itemDelegate_closeEditor()
{
    if (!m_objPtr)  return;
...
void
WtTplCtorFn::repBaseClassValItem()
{
...
bool
WtTplCtorFn::nameCheckDuplication(std::string const &  fnName)
{
    bool  res = false;
...
Function *
WtTplCtorFn::getObjPtr() const
{
    return m_objPtr;
}

void
WtTplCtorFn::setObjPtr(Function *  value)
{
    m_objPtr = value;
    if (m_objPtr) {
